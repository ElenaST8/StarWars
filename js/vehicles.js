...
async function getVehicles(page, create = false) {
  try {
...
function showAllVehicles(data) {
  ...
  data.forEach((element) => {
    ...
    let str = `
      <div class="card mb-3 card-universal">
  ...
  bindVehicleEvents(data);
}

function bindVehicleEvents(data) {
...
function activePagination() {
  ...
  pages.forEach((page) => {
    page.addEventListener("click", function () {
      ...
      getVehicles(parseInt(this.textContent));
    });
  });
}
